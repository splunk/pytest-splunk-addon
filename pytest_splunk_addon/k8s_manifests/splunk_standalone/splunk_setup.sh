...
if [ -f "$TEST_RUNNER_DIRECTORY/tests/src/$SPLUNK_ADDON" ]; 
then
    kubectl cp $TEST_RUNNER_DIRECTORY/tests/src/$SPLUNK_ADDON nginx:/usr/share/nginx/html -n $NAMESPACE_NAME
    echo "        - http://nginx//$SPLUNK_ADDON" >> "$(dirname -- "$0")"/addon_information_updated.yaml;
else
    echo "Unable to found $SPLUNK_ADDON in tests/src"
fi
