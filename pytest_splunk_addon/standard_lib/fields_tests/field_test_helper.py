...
class FieldTestHelper(object):
    ...
    def get_exc_message(self):
        ...
        if len(self.parsed_result["fields"]) == 1:
            return (
                f"Field = {self.parsed_result['fields'][0]['field']}"
                f"\nEvent Count = {self.result.get('event_count')}"
                f"\nField Count = {self.parsed_result['fields'][0]['field_count']}"
                f"\nValid Field Count = {self.parsed_result['fields'][0]['valid_field_count']}"
                f"\nSearch = {self.search}"
                f"\n"
                f"\nField properties :: {self.parsed_result['fields'][0]['field'].get_properties()}"
            )
        else:
            exc_message = "\n{:<20}{:<20}{:<20}".format(
                     "Field", "Field Count", "Valid Field Count"
                )
            exc_message += "\n" + "-"*50
            for each_field in self.parsed_result["fields"]:
                exc_message += (
                    ...
                    each_field.get("invalid_field_count", each_field["field_count"])
                ))
            exc_message += "\n" + "-"*50
            exc_message += (
