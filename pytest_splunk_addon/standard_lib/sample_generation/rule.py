...
class IntRule(Rule):
    ...
    def apply(self, sample):
        ...
        tokenised_sample = re.sub(self.name, str(randint(int(replace_range[0]), int(replace_range[1]))), sample.sample_raw_data)
...
class FloatRule(Rule):
    ...
    def apply(self, sample):
        ...
        tokenised_sample = re.sub(self.name, str(replace_float), sample.sample_raw_data)
        return tokenised_sample
...
class ListRule(Rule):
    ...
    def apply(self, sample):
        ...
        tokenised_sample = re.sub(self.name, str(choice(value_list)), sample.sample_raw_data)
        return tokenised_sample
...
class StaticRule(Rule):
    ...
    def apply(self, sample):
        tokenised_sample = re.sub(self.name, self.replacement, sample.sample_raw_data)
        return tokenised_sample
...
class FileRule(Rule):
    ...
    def apply(self, sample):
        ...
        try:
            ...
            tokenised_sample = re.sub(self.name, choice(lines), sample.sample_raw_data)
            return tokenised_sample
...
class TimeRule(Rule):  
    ...
    def apply(self, sample):
        if r"%s" in self.replacement:
            tokenised_sample = re.sub(self.name, self.replacement.replace(r'%s', str(int(time()))), sample.sample_raw_data, flags=re.MULTILINE)
            return tokenised_sample

        if r"%e" in self.replacement:
            print(r"timestamp -> %e has compatibility issues (works in linux only). Please replace it with %d.")
        tokenised_sample = re.sub(self.name, strftime(self.replacement.replace(r'%e', r'%d')), sample.sample_raw_data, flags=re.MULTILINE)
        return tokenised_sample
...
class ipv4Rule(Rule):
    ...
    def apply(self, sample):
        tokenised_sample = re.sub(self.name, ".".join(map(str, (randint(0, 255) for _ in range(4)))), sample.sample_raw_data)
        return tokenised_sample
...
class ipv6Rule(Rule):
    ...
    def apply(self, sample):
        M = 16**4
        tokenised_sample = re.sub(self.name, ":".join(("%x" % randint(0, M) for i in range(6))), sample.sample_raw_data)
        return tokenised_sample
...
class GuidRule(Rule):
    ...
    def apply(self, sample):
        tokenised_sample = re.sub(self.name, str(uuid.uuid4()), sample.sample_raw_data)
        return tokenised_sample
