    ...
    def generate_tests(self, app_path, config_path, test_type):
        ...
        for tokenized_event in tokenized_events:
            ...
            if tokenized_event.metadata.get("host_type") in ('plugin', None):
                ...
                if not hosts:
                    raise_warning("For stanza '{}' host_type is 'event' and 'host' not found in key fields. So no tests are generated for the stanza.".format(tokenized_event.sample_name))
                    continue
            else:
