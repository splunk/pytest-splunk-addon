...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "splunk_app_internal_errors: Check Errors in logs"
    )
    config.addinivalue_line("markers", "splunk_app_searchtime: Test search time only")
    config.addinivalue_line(
        "markers", "splunk_app_fields: Test search time fields only"
    )
    config.addinivalue_line(
        "markers", "splunk_app_negative: Test search time negative scenarios only"
    )
    config.addinivalue_line("markers", "splunk_app_tags: Test search time tags only")
    config.addinivalue_line(
        "markers", "splunk_app_eventtypes: Test search time eventtypes only"
    )
    config.addinivalue_line("markers", "splunk_app_cim: Test CIM compatibility only")
    config.addinivalue_line(
        "markers", "splunk_app_cim_fields: Test CIM required fields only"
    )
    config.addinivalue_line(
        "markers",
        "splunk_app_cim_fields_not_extracted: Test CIM fields for mapped datamodels which should not be extracted in splunk. i.e expected event count for the fields: 0",
    )
    config.addinivalue_line(
        "markers",
        "splunk_app_cim_fields_not_allowed: Test CIM fields for mapped datamodels whose extractions should not be defined in the addon.",
    )
