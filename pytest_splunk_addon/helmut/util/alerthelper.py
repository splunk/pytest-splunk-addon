...
class AlertHelpers(object):
    ...
    def createbasicAlert(self, name, subject, search='index=_internal',  # replace index=_internal with the other search
                         cron_schedule='*/1 * * * *'):
        """
        ...
        request_args['search'] = search  # replace hardcoding with current working directory
    ...
    def setup_mail_server(self, splunk, user, password, mail_server, mode='None'):
        ...
        if (mode == 'SSL'):
            ...
            request_args['mailserver'] = const.TestConstants['SMTP_SERVER'].format('465')
        ...
        self.make_http_request(splunk, request_type, request_url, request_args, splunk.username, splunk.password)
        splunk.execute('splunk restart')

    def updatePaperSettings(self, paper_size='Letter', paper_orientation='Portrait', splunk_logo=0):
        """
    ...
    def get_paper_orientation(self, splunk, alertname):
        ...
        content = self.make_http_request(splunk, request_type, request_url, req_args, splunk.username, splunk.password)
        parsedresponse = json.loads(content)
        paper_orientation = str(parsedresponse['entry'][0]['content']['action.email.reportPaperOrientation'])
    ...
    def get_paper_size(self, splunk, alertname):
        ...
        content = self.make_http_request(splunk, request_type, request_url, req_args, splunk.username, splunk.password)
        parsedresponse = json.loads(content)
        paper_size = str(parsedresponse['entry'][0]['content']['action.email.reportPaperSize'])
        return paper_size
    ...
    def get_triggered_alert_count(self, splunk, alert_name, password='changeme'):
        ...
        try:
            ...
            content = self.make_http_request(splunk, request_type, request_url, req_args, splunk.username,
                                             splunk.password)
            parsedresponse = json.loads(content)
            triggered_alert = int(parsedresponse['entry'][0]['content']['triggered_alerts'])
            return triggered_alert
        except Exception as e:
            LOGGER.error("Exception when get triggered_alert:" + repr(e))
            return None

    def trigger_alerts(self, splunk, mail_server, user, password, emailMode='None', search=None,
                       filename='alertconfig.conf', hostname=None):
        """
        ...
        f = conf.readlines()
        self.setup_mail_server(splunk,
                               user,
                               password,
                               mail_server,
                               mode=emailMode)  # for mode get pytest parameter
        ...
        for line in f:
            ...
            self.createbasicAlert(name=elements[0], cron_schedule=elements[1],
                                  subject=elements[0], search=search)
            ...
            self.updatePaperSettings(paper_size=elements[8],
            ...
            self.createAlert(splunk, context=elements[12].rstrip('\n'),
                             user=elements[11].rstrip('\n'))

    def create_new_app(self, splunk, appname, appcontext, user, splunk_user='admin', splunk_pwd='changeme'):
        ...
        request_type = 'POST'
        self.make_http_request(splunk, request_type, app_url, app_params, splunk_user=splunk_user,
                               splunk_pwd=splunk_pwd)

    def create_new_user(self, splunk, newuser, pwd, roles, splunk_user='admin', splunk_pwd='changeme'):
        #        app_params = {'name':'nithya','app':'search','user':'admin',}
    ...
    def get_splunk(self, TEST_DIR, branch=None, product=None, build=None):
        ...
        Splunk  instance
        '''
        splunk_home = TEST_DIR + os.sep + 'test_installs' + os.sep + 'temp_splunk_instance'
        splunk_instance = SplunkFactory.getSplunk(splunk_home)
        splunk_instance.install_nightly(branch=branch, package_type=product, build=build)
    ...
    def one_shot_upload(self, splunk, user, appcontext, one_shot_file, splunk_user='admin', splunk_pwd='changeme'):
        print('this method will upload the data input file needed for \
        ...
        request_type = 'POST'
        self.make_http_request(splunk, request_type, oneshot_url, oneshot_args, splunk_user=splunk_user,
                               splunk_pwd=splunk_pwd)
    ...
    def make_http_request(self, splunk, request_type, request_url, request_args,
        ...
        try:
            ...
            LOGGER.error("Http error code is ({0}): {1} : {2}".format(err.code,
                                                                      err.errno,
                                                                      err.strerror))
        except http.client.ResponseNotReady as e:
            time.sleep(5)
            LOGGER.warn("httplib.ResponseNotReady error happen, retry once. {e}".format(e=e))
            # retry one time
            try:
                restconn = splunk.create_logged_in_connector(contype=Connector.REST,
                                                             username=splunk_user,
