...
class YMLParser(object):
    ...
    def extract_key_values(self, yaml_file):
        ...
        if not yaml_file in YAML_HOLDER:
            ...
            for item in yaml_dict:
                LOGGER.info('item in yaml file: {key}: {value}'.format(
    ...
    def get_hosts(self, yaml_file):
        ...
        for host in host_list:
            hosts.add_host(Host(host_name=host, ssh_user=config['ssh_user'], ssh_password=config['ssh_password'],
                                splunk_home=config['splunk_home'],
                                ssh_domain=config['ssh_domain'], ssh_port=config.get('ssh_port', 22),
