...
class CloudSplunk(Splunk):
    def __init__(self, name=None, splunkd_scheme=None, splunkd_host=None, splunkd_port='',
                 web_scheme=None, web_host=None, web_port=None,
        ...
        if not (web_scheme and web_host):
            try:
                ...
                server_web_scheme = 'http' if server_settings['enableSplunkWebSSL'] == "0" else 'https'
                ...
                self.logger.warning("No sufficient permissions to qury server settings.")
    ...
    def web_base(self):
        ...
        return "{scheme}://{host}:{web_port}".format(
            ...
            host=self.web_host(), web_port=self.web_port())

    def create_connector(self, contype=None, username=None, password=None,
                         *args, **kwargs):
        """
        @param contype:
        ...
        @return:
        """
        if username and username != self.username and password and password != self.password:
            raise CloudSplunkConnectorException()
        return super(CloudSplunk, self).create_connector(contype=contype, username=username,
        ...
        while True:
            ...
            if result == resultPrev:
                if (now - resultSameSince) > secondsToStable:  ### we have stable state
                    self.logger.info(
                        'Achieved stable state for search %s with totalEventCount=%s' % (search_string, result))
                    return result  # returns the final event count...
                if resultSameSince == sys.maxsize:  ### our first time in what could become stable state
                    self.logger.debug(
                        'Possibly entering stable state for search %s at totalEventCount=%s' % (search_string, result))
                    ...
                    self.logger.debug('Confirming putative stable at totalEventCount=%s for search_string %s ' % (
                        result, search_string))
            else:  ### we do NOT have stable state
                self.logger.debug('Flux at totalEventCount=%s for search_string %s; delta +%s' % (
                    result, search_string, (result - resultPrev)))
                resultPrev = result
                resultSameSince = sys.maxsize
            lastPolledAt = now

    def ensure_event_count(self, search_string, expect_count, retry=3, retry_interval=10):
        '''
        @param search_string:
        @param retry:
        ...
        while retry > 0:
            ...
            if event_count == expect_count:
                ('event count met. event_count={event_count}'.format(event_count=str(event_count)))
                return True
            else:
                self.logger.debug(
                    'wait event count to met. event_count={event_count}'.format(event_count=str(event_count)))
            time.sleep(retry_interval)
            retry = retry - 1
        self.logger.debug(
            'Timeout wait event count to met. event_count={event_count}'.format(event_count=str(event_count)))
        return False


class CloudSplunkConnectorException(Exception):
    message = "Don't pass username/password to connector. Helmut allows only one user per CloudSplunk instance." \
