...
from pytest_splunk_addon.helmut.exceptions.command_execution import CommandExecutionFailure
...
class Splunk(with_metaclass(ABCMeta, Logging)):
    ...
    def create_connector(self, contype=None, username=None, password=None,
        ...
        conn = self._CONNECTOR_TYPE_TO_CLASS_MAPPINGS[contype](self, *args,
                                                               **kwargs)

        connector_id = self._get_connector_id(
            contype=contype, user=conn.username)

        if connector_id in list(self._connectors.keys()):
            self.logger.warning("Connector {id} is being replaced".format(
                id=connector_id))
    ...
    def create_logged_in_connector(
        ...
        '''
        contype = contype or Connector.SDK
        conn = self.create_connector(contype, username=username,
    ...
    def default_connector(self):
        ...
        if self._default_connector is None:
            self._default_connector = self.create_logged_in_connector(
                set_as_default=True, username=self.username,
    ...
    def connector(self, contype=None, username=None, password=None):
        ...
        if contype == 'REST':
            rest_conn = self.create_logged_in_connector(contype=Connector.REST, username=username, password=password)
    ...
    def get_rip(self, owner=None, app=None, username=None, password=None):
        ...
        self.create_logged_in_connector(contype=Connector.REST,
                                        username=username,
                                        password=password,
                                        owner=owner,
                                        app=app)
