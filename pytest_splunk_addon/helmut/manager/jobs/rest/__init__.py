...
from pytest_splunk_addon.helmut.manager.jobs import Jobs, JobNotFound, PATH_PERFIX, CONTROL, EVENTS, RESULTS, \
    RESULTS_PREVIEW, SEARCHLOG, \
    SUMMARY, TIMELINE
...
class RESTJobsWrapper(Jobs):
    ...
    def _create(self, query, **kwargs):
        ...
        args = user_args.copy()
        args.update(kwargs)
        response, content = self.connector.make_request('POST', url, args, {'output_mode': 'json'})
...
class RestJob(object):
    ...
    def update_search(self, **kwargs):
        ...
        url = PATH_PERFIX + sid
        req_args = {'output_mode': 'json'}
        response, content = self.connector.make_request("POST", url, body=kwargs, urlparam=req_args)
        assert response['status'] == "200"
    ...
    def delete_search(self, **kwargs):
        ...
        url = PATH_PERFIX + sid
        req_args = {'output_mode': 'json'}
        response, content = self.connector.make_request('DELETE', url, body=kwargs, urlparam=req_args)
        assert response['status'] == "200"
    ...
    def get_search_results(self, **kwargs):
        ...
        response, content = self.connector.make_request('GET', url, urlparam=args, log_response=False)
        assert response['status'] == "200"
        return content
