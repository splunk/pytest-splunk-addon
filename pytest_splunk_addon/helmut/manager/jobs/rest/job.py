...
class RESTJobWrapper(Job):
    ...
    def get_event_count(self):
        result = self.raw_rest_job.refresh()
        return int(result['content']['eventCount']) if 'eventCount' in result['content'] else 0

    def get_scan_count(self):
        result = self.raw_rest_job.refresh()
        return int(result['content']['scanCount']) if 'scanCount' in result['content'] else 0

    def get_event_available_count(self):
        result = self.raw_rest_job.refresh()
        return int(result['content']['eventAvailableCount']) if 'eventAvailableCount' in result['content'] else 0

    def get_result_count(self):
        result = self.raw_rest_job.refresh()
        return int(result['content']['resultCount']) if 'resultCount' in result['content'] else 0
    ...
    def set_ttl(self, value):
        ...
        assert "The ttl of the search job was changed to {value}".format(value=value) in response['messages'][0]['text']
        return self
    ...
    def get_latest_time(self):
        result = self.raw_rest_job.refresh()
        return result['content']['latestTime'] if 'latestTime' in result['content'] else None
    ...
    def set_priority(self, value):
        ...
        response = self.raw_rest_job.control_search(action="setpriority", priority=value)
        assert "The search job's priority was changed" in response['messages'][0]['text']
        return self
