...
from pytest_splunk_addon.helmut.manager.saved_searches.sdk.saved_search import SDKSavedSearchWrapper
...
class SDKSavedSearchesWrapper(SavedSearches):
    ...
    def create(self, saved_search_name, query, **kwargs):
        ...
        try:
            ...
            self.connector.service.saved_searches.create(saved_search_name, query, **kwargs)
        except HTTPError as err:
            # Saved search already exists
            if not err.status == 409:
                raise
            self.logger.warn("Saved search '%s' already exists. HTTPError: %s" %
    ...
    def items(self):
        ...
        return [SDKSavedSearchWrapper(self.connector, saved_search) for saved_search in saved_searches]
