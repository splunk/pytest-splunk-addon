...
from pytest_splunk_addon.helmut.manager.indexes import IndexNotFound, PATH_PERFIX, COUNT_OFFSET, DISABLE, \
    SYSTEM_MESSAGE, RESTART, ENABLE, \
    ROLL_HOT_BUCKETS, OperationError
...
class RestIndex(object):
        ...
        try:
            ...
            while not Done and datetime.now() < start + diff:
                ...
                if int(res['content']['totalEventCount']) == 0:
                    Done = True
            if not Done:
                raise_(OperationError, "Cleaning index %s took longer than %s seconds; timing out." % \
                       (self._name, timeout))
        finally:
    ...
    def update(self, **kwargs):
        name = self.encode_name()
        kwargs = dict([normalize_to_str(k), normalize_to_str(v)] for k, v in kwargs.items())
        url = PATH_PERFIX + name
        req_args = {'output_mode': 'json'}
    ...
    def restart(self, timeout=None):
        ...
        response, content = self.connector.make_request('POST', url, user_args, {'output_mode': 'json'})
        assert response['status'] == '201'
        response, content = self.connector.make_request('POST', RESTART, {'output_mode': 'json'})
        assert response['status'] == '200'
    ...
    def login(self, connector):
        if hasattr(connector, "is_logged_in") and connector._attempt_login_time > 0 and not connector.is_logged_in():
            connector.login()
