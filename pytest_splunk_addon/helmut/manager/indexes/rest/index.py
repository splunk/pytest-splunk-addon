...
class RESTIndexWrapper(Index):
    ...
    def wait_for_event_count(self, ecount, timeout):
        ...
        while not done and counter > 0:
            ...
            if event_number > ecount:
                ...
                if event_number != previous_event_number:
                    self.logger.info('Index {name} events count is {now},previous events count is {pre}'.format(
                        name=self.name,
                        now=event_number,
                        pre=previous_event_number))
        ...
        if counter != 0:
            self.logger.info('Indexing (%s) completed in %s seconds.'
                             % (self.name, (timeout - counter)))
        else:
            self.logger.warn('Indexing (%s) did not complete within %s seconds.The events number is %s'
    ...
    def edit(self, **kwargs):
        kwargs = dict([normalize_to_unicode(k), normalize_to_unicode(v)] for k, v in kwargs.items())
