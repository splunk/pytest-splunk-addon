...
from pytest_splunk_addon.helmut.util.string_unicode_convert import normalize_to_str, normalize_to_unicode
import json
...
class RESTConfWrapper(Conf):
    ...
    def stanzas_list(self, ):
        ...
        return [RestStanza(self.connector, self._raw_rest_conf, stanza_name) for stanza_name in stanza_list]
    ...
    def _create_stanza(self, stanza_name, **values):
        ...
        values = dict([normalize_to_str(k), normalize_to_str(v)] for k, v in values.items())
        ...
        response, content = self.connector.make_request('POST', url, user_args, {'output_mode': 'json'})
    ...
    def _delete_stanza(self, stanza_name):
        stanza_name = normalize_to_str(stanza_name)
        url = PATH_PERFIX + self._raw_rest_conf.path + '/{stanza_name}'.format(stanza_name=stanza_name)
    ...
    def create_stanza(self, stanza_name, values=None):
        values = values or {}
        values = dict([normalize_to_unicode(k), normalize_to_unicode(v)] for k, v in values.items())
        ...
        try:
            ...
            return RESTStanzaWrapper(self,
                                     self._create_stanza(stanza_name, **values))
        except HTTPError as h:
            self.logger.warn("Stanza '%s' already existed in %s.conf. "
                             "HTTPError message: %s" % (stanza_name,
                                                        self.name, h))
...
class RestStanza(object):
    ...
    def _content(self):
        ...
        url = PATH_PERFIX + self.rest_conf.path + '{stanza_name}'.format(stanza_name=name)
        req_args = {'output_mode': 'json'}
        response, content = self.connector.make_request("GET", url, req_args)
        assert response['status'] == "200"
        parsed_content = json.loads(content)
