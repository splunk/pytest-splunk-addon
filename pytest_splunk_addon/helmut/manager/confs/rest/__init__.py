...
from pytest_splunk_addon.helmut.manager.confs import PATH_PERFIX, PATH_CONF, PATH_PROPERTIES
...
class RESTConfsWrapper(Confs):
    ...
    def create(self, conf_name):
        ...
        # create(conf_name)
        return RESTConfWrapper(self.connector,
                               self._create(conf_name))

    def items(self):
        conf_names = self.list()
        #        for c in conf_names:
        #            print c.name
        return [RESTConfWrapper(self.connector, conf_name)
