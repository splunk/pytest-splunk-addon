...
def splunk_docker(
    ...
    import subprocess
    p = subprocess.Popen(f"curl {request.config.getoption('splunk_hec_scheme')}://{splunk_info['forwarder_host']}:{splunk_info['port_hec']}/services/collector/event -H 'Authorization: Splunk {request.config.getoption('splunk_hec_token')}' -d '{{\"event\": \"hello world\"}}'", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
