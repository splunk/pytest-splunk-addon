...
class ModularActionQutils(object):
    ...
    CAM_QUEUE_URI = (
        "/servicesNS/nobody/Splunk_SA_CIM/storage/collections/data/cam_queue"
    )
    QUEUE_PAYLOAD_MAP = {
        "time": (
        ...
        "info": (lambda x: isinstance(x, basestring), "must be a string of any length"),
        "worker": (
            lambda x: isinstance(x, basestring) and x,
            "must be a string with positive length",
        ),
        "settings": (validate_settings, "must be a json encoded nonempty dictionary"),
    }
    ...
    def is_api_key_formatted(api_key):
        if isinstance(api_key, basestring) and ModularActionQutils.API_KEY_RE.match(
            api_key
        ):
            return True
    ...
    def build_key(item):
        try:
            ...
            return "{0}@@{1}".format(
                item["worker"], hashlib.sha1(hashtxt.encode("utf-8")).hexdigest()
            )
    ...
    def get_unsave_sids(dequeued_items, sid_items):
        ...
        """
        try:
            dequeued_keys = [x["_key"] for x in dequeued_items if x.get("_key")]
            sids = set([x["sid"] for x in sid_items if x.get("sid")])
            bad_sids = set(
                [
                    x["sid"]
                    for x in sid_items
                    if (x.get("_key") and x["_key"] not in dequeued_keys)
                ]
            )
    ...
    def get_work(self, worker):
        if not (isinstance(worker, basestring) and worker):
            raise ModularActionQueueBR("Must specify a worker in order to get work")
        ...
        try:
            r, c = rest.simpleRequest(uri, sessionKey=self.session_key, method="GET")
        except Exception as e:
            self.logger.exception(e)
            raise ModularActionQueueISE("Unable to retrieve cam_queue")
    ...
    def get_results(self, key):
        if not (isinstance(key, basestring) and key):
            raise ModularActionQueueBR("Must specify a key in order to get results")

        if not self.session_key:
            raise AuthorizationFailed

        # get cam_queue
        uri = "{0}/{1}".format(self.CAM_QUEUE_URI, quote(key, safe=""))
        ...
        if not results_file.startswith(
            make_splunkhome_path(["var", "run", "splunk", "dispatch"])
        ):
            self.logger.error("Unable to load results_file: %s", results_file)
            raise ModularActionQueueISE("Unable to load results_file")
    ...
    def validate_queue_payload(self, payload, max_items):
        ...
        for i, jsonitem in enumerate(jsonargs):
            if not isinstance(jsonitem, dict):
                raise ModularActionQueueBR("Payload items must be valid dictionaries")
            ...
            for key, val in jsonitem.items():
                ...
                if not exp(val):
                    raise ModularActionQueueBR(
                        'Payload item "{0}" was invalidated: {1}'.format(key, reason)
                    )
    ...
    def validate_dequeue_payload(self, payload, worker):
        ...
        for jsonitem in jsonargs:
            if not isinstance(jsonitem, basestring):
                raise ModularActionQueueBR("Payload items must be valid strings")
    ...
    def save_work(self, save_sids):
        ...
        for save_sid in save_sids:
            try:
                rest.simpleRequest(
                    "search/jobs/{0}/control".format(quote(save_sid, safe="")),
                    postargs={"action": "save"},
                    sessionKey=self.session_key,
                    raiseAllErrors=True,
                )

                self.logger.info("Successfully saved job: %s", save_sid)
            except Exception as e:
                self.logger.exception(e)
                self.logger.warn("Unable to save job: %s", save_sid)
    ...
    def unsave_work(self, query):
        ...
        try:
            ...
            self.logger.exception(e)
            raise ModularActionQueueISE("Unable to retrieve work from cam_queue")
        ...
        sid_query = {
            "$or": [{"sid": x.get("sid")} for x in dequeued_items if x.get("sid")]
        }
        ...
        try:
            r, c = rest.simpleRequest(
                sid_uri, sessionKey=self.session_key, method="GET"
            )
        except Exception as e:
            self.logger.exception(e)
            raise ModularActionQueueISE("Unable to retrieve SIDs from cam_queue")
        ...
        unsave_sids = self.get_unsave_sids(dequeued_items, sid_items)

        for unsave_sid in unsave_sids:
            try:
                rest.simpleRequest(
                    "search/jobs/{0}/control".format(quote(unsave_sid, safe="")),
                    postargs={"action": "unsave"},
                    sessionKey=self.session_key,
                    raiseAllErrors=True,
                )

                self.logger.info("Successfully unsaved job: %s", unsave_sid)
            except Exception as e:
                self.logger.exception(e)
                self.logger.warn("Unable to unsave job: %s", unsave_sid)
    ...
    def delete_work(self, query):
        ...
        try:
            r, c = rest.simpleRequest(uri, sessionKey=self.session_key, method="DELETE")
